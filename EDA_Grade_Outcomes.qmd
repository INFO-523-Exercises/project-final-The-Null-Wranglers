...
```{r}
clean_dew_data<- read.csv("data/clean_dew_data.csv")
# Calculating means
means <- c(
  mean(clean_dew_data$Total.Student.Count)
  
)

# Creating a data frame
means_data <- data.frame(
  Variable = c("Total Student Count"),
  Mean_Value = means
)
# Creating a kable table
kable(means_data,  caption = "Mean of Enrollment, Pass grades, DEW Grades ")
```
...
means_data <- data.frame(
  ...
  Mean_Value = c(
    ...
    mean(clean_dew_data$PASS_GRADE_COUNT),
    mean(clean_dew_data$D_GRADE_COUNT),
    mean(clean_dew_data$FAIL_GRADE_COUNT),
    mean(clean_dew_data$WITHDRAW_GRADE_COUNT),
    mean(clean_dew_data$DEW_COUNT)
  )
)
...
college_means_data <- clean_dew_data %>%
  group_by(College) %>%
